...
impl TryParse<FILETIME> for Parser<'_> {
    fn try_parse(&mut self, name: &str) -> Result<FILETIME, ParserError> {
        let prop_info = self.find_property(name)?;
        let prop_info: &PropertyInfo = prop_info.borrow();

        if prop_info.property.in_type() != TdhInType::InTypeFileTime {
            return Err(ParserError::InvalidType);
        }

        if std::mem::size_of::<FILETIME>() != prop_info.buffer.len() {
            return Err(ParserError::LengthMismatch);
        }

        let res: FILETIME = unsafe { std::ptr::read(prop_info.buffer.as_ptr() as *const _) };
